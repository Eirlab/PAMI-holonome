...
// #include "robot_holonomic_eirbot.h"
...
// ENCODEUSE encoder1(M1C1, M1C2); // Create an instance of the ENCODEUSE class
// ENCODEUSE encoder2(M2C1, M2C2); // Create an instance of the ENCODEUSE class
// ENCODEUSE encoder3(M3C1, M3C2); // Create an instance of the ENCODEUSE class
...
void setup() {
  ...
  // motor_driver.configMotor(1, 1, 2, 0, 0);  // set up motor 1 to use half bridges 1,2 and pwm channel 0
  // motor_driver.configMotor(2, 3, 4, 1, 0);  // set up motor 2 to use half bridges 3,4 and pwm channel 1
  // motor_driver.configMotor(3, 5, 6, 2, 0);  // set up motor 3 to use half bridges 5,6 and pwm channel 2
...
void updateMotors() {
  ...
  if (measure.RangeStatus != 4) {  // phase failures have incorrect dat
    ...
    if (measure.RangeMilliMeter < 100) {
      ...
      if(abs(lastAngle - angle) > 1 or abs(lastAlpha - alpha) > 1) {
...
void updateMotorsDifferential() {
  ...
  float m3 = vy;               // Moteur 1
  float m2 = vx * cos(2*PI/3) + vy * sin(2*PI/3);     // Moteur 2
  float m1 = vx * cos(4*PI/3) + vy * sin(4*PI/3);     // Moteur 3
...
//       motor_driver.setMotor(1, 100, DRV89xx_FORWARD);   // ramp up speed to full, then wrap around
//     motor_driver.setMotor(2, 100, DRV89xx_FORWARD);
//     // motor_driver.setMotor(3, i, DRV89xx_FORWARD);

//   // Serial.println("avance");
//   // for (byte i=0;i<255;i++) {
//   //   motor_driver.setMotor(1, i, DRV89xx_FORWARD);   // ramp up speed to full, then wrap around
//   //   motor_driver.setMotor(2, i, DRV89xx_FORWARD);
//   //   motor_driver.setMotor(3, i, DRV89xx_FORWARD);
...
//   // motor_driver.setMotor(1, 0, DRV89xx_BRAKE);   
//   // motor_driver.setMotor(2, 0, DRV89xx_BRAKE);
//   // motor_driver.setMotor(3, 0, DRV89xx_BRAKE);
//   // motor_driver.updateConfig();  // writes new motor values to driver
//   // vTaskDelay(1000);                    // wait for 1sec
  
//   // Serial.println("recul");
//   // for (byte i=0;i<255;i++) {
//   //   motor_driver.setMotor(1, i, DRV89xx_REVERSE);   // ramp up speed to full, then wrap around
//   //   motor_driver.setMotor(2, i, DRV89xx_REVERSE);
//   //   motor_driver.setMotor(3, i, DRV89xx_REVERSE);
//   //   motor_driver.updateConfig();  // writes new motor values to driver
...
//   // Serial.println("stop");  
//   // motor_driver.disableMotor(1);   // disconnect bridge
//   // motor_driver.disableMotor(2);   // disconnect bridge
//   // motor_driver.disableMotor(3);   // disconnect bridge
//   // motor_driver.updateConfig();  // writes new motor values to driver
...
//   // for(int i=0; i<5; i++){
//   //   int newEn1 = encoder1.get_counter();
//   //   int newEn2 = encoder2.get_counter();
//   //   int newEn3 = encoder3.get_counter();

//   //   Serial.print("Encoder 1: ");
//   //   Serial.println(newEn1);
//   //   Serial.print("Encoder 2: ");
//   //   Serial.println(newEn2);
//   //   Serial.print("Encoder 3: ");
//   //   Serial.println(newEn3);
//   //   Serial.println("--------------");
...
//   //     if(mode == 'D') {
//   //       updateMotorsDifferential();
//   //     } else {
//   //       updateMotorsHolonomic();
//   //     }
